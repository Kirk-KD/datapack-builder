...
import {EditorStateLowering} from './editorStateLowering.ts'
...
export class LoweringPass implements IrVisitor<LoweredResult> {
  ...
  visitItemStack(node: ItemStackNode): LoweredResult {
    return new EditorStateLowering(this, node.sourceBlockId).compileEditorState({
      compiler: 'item_stack',
      error: false,
      data: node.itemStackData
    }, {})
  }
